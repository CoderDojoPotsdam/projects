...
import de.hamster.debugger.model.Territorium;import de.hamster.model.HamsterException;import de.hamster.model.HamsterInitialisierungsException;import de.hamster.model.HamsterNichtInitialisiertException;import de.hamster.model.KachelLeerException;import de.hamster.model.MauerDaException;import de.hamster.model.MaulLeerException;import de.hamster.debugger.model.Hamster;class Superhirn extends AllroundHamster {
  ...
  Superhirn() {
    super(0, 0, Hamster.OST,
          Superhirn.RUNDEN * 
            (Superhirn.ANZAHL_ZAHLEN * Superhirn.MAX_ZAHL + 4));
    ...
    for (int z=0; z<Superhirn.ANZAHL_ZAHLEN; z++) {
      this.geheimeZahlen[z] = 
        Zufall.naechsteZahl(Superhirn.MAX_ZAHL);
    }
  ...
  boolean ueberpruefeSpielfeld() {
    return 
      Territorium.getAnzahlReihen() >= Superhirn.RUNDEN &&
  ...
  void ergebnisBerechnen() {
    ...
    for (int i=0; i<Superhirn.ANZAHL_ZAHLEN; i++) {
      if (!posErledigt[i]) {
        ...
        for (int j=0; 
             j<Superhirn.ANZAHL_ZAHLEN && !jErledigt; 
             j++) {
          if (i != j && !vorErledigt[j] &&
  ...
  boolean ergebnisAuslegen(int runde) {
    ...
    // Zahlen korrekt sind und an der korrekten Position stehen 
    this.gib(this.korrektePosition);
    this.schreib(this.korrektePosition + " korrekte Positionen");
    this.vor();
    
    // dann werden so viele Koerner ausgelegt, wie
    // Zahlen korrekt sind, aber an der falschen Position stehen 
    this.gib(this.korrektesVorkommen);
    this.schreib(this.korrektesVorkommen + " korrekte Vorkommen");
    
    if (this.korrektePosition == Superhirn.ANZAHL_ZAHLEN) {
      this.schreib("Herzlichen Glueckwunsch. Du hast gewonnen!");
      return true;
