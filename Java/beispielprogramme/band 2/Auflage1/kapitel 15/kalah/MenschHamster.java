  ...
  implements Spieler 
{
  ...
  public MenschHamster(boolean istSpielerA,
                       Spielregeln regeln) {
  ...
  public Spielzug naechsterSpielzug() {
    ...
    int mulde = 
      this.liesZahl("Spieler " + this.liefereSpielerTyp() +
                    ": Welche Mulde?");
    Spielzug zug = new Spielzug(mulde);
    boolean ok = this.regeln.istSpielzugOk(this.istSpielerA, zug);
    while (!ok) {
      mulde = this.liesZahl("Fehler!\n Spieler " +
                            this.liefereSpielerTyp() +
                            ": Bitte korrekte Mulde angeben!");
      zug = new Spielzug(mulde);
      ok = this.regeln.istSpielzugOk(this.istSpielerA, zug);
    }
  ...
  protected void gotoMulde(int muldenNummer) {
    if (this.istSpielerA) {
      ...
      this.vor(muldenNummer - Spielbrett.ANZAHL_MULDEN_PRO_SPIELER);
    }
  }
  ...
  protected void verteileKoerner(int anzahl, int aktuelleMulde) {
    ...
    while (anzahl > 0) {
      if (aktuelleMulde == Spielbrett.KALAH_A - 1 ||
        ...
        this.linksUm();
        this.vor();
        if (this.istSpielerA && aktuelleMulde == Spielbrett.KALAH_A - 1 ||
            !this.istSpielerA && aktuelleMulde == Spielbrett.KALAH_B - 1
      ...
      aktuelleMulde = (aktuelleMulde + 1) % Spielbrett.ANZAHL_MULDEN;
    }
    
    // nun ueberpruefen, ob evtl. einige Koerner ins eigene Kalah
    // abgelegt werden duerfen
    if (this.istSpielerA && 
        0 <= aktuelleMulde && aktuelleMulde < Spielbrett.KALAH_A &&
        Territorium.getAnzahlKoerner(this.getReihe(), 
                                     this.getSpalte()) == 1 &&
        Territorium.getAnzahlKoerner(this.getReihe()+2, 
                                     this.getSpalte()) > 0
       ) {
      this.nimm();   // das eine Korn der aktuellen Mulde fressen
      this.linksUm();
      this.vor();
      this.vor();
      this.nimmAlle(); // gegenueberliegende Mulde leer fressen
      this.gotoKalahA();
  ...
  protected void begibDichAufAusgangsKachel() {
    if (this.istSpielerA) {
      this.gotoKachel(0, Spielbrett.ANZAHL_MULDEN_PRO_SPIELER + 1);
  ...
  protected void gotoKalahB() {
    this.gotoKachel(1, Spielbrett.ANZAHL_MULDEN_PRO_SPIELER + 1);
  }
}